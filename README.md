...
An Azure-styled gateway for Dify that maps deployment IDs directly to model names and speaks the OpenAI **Responses** and **Embeddings** APIs with a default `api-version=fake` (overridable per credential). The provider exposes Large Language Models (LLM) and text embedding models behind a single credential set.
...
All endpoints follow the Azure OpenAI pattern (defaulting to `api-version=fake` unless overridden):
...
3. Optional **API version** – overrides the query-string `api-version` (default `fake`).
4. Optional **deployment path toggle** – set `use_deployid=false` if your gateway exposes `/chat/completions` instead of `/deployments/{model}`.
5. Optional **timeouts** — synchronous (default 60s) and streaming (default 300s) read timeouts.
6. Optional **pseudo-SSE chunk count** — how many slices to emit for non-OpenAI responses (default 2).
